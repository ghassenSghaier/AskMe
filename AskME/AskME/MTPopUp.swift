...
class MTPopUp: UIView
{
    ...
    override init(frame: CGRect)
    {
        ...
        viewHeaderPopup.backgroundColor = UIColor(hexString: "#f39c12")
    ...
    func show(complete : ((Int)->())? , view: UIView,animationType: NSString,strMessage: String,btnArray: NSArray,strTitle: String = "")
    {
        ...
        for strBtnTitle in btnArray
        {
            ...
            btnOk.backgroundColor = UIColor(hexString: "#f39c12")
        ...
        viewPopUp.frame = CGRect(x: viewPopUp.frame.origin.x ,y: CGFloat(popUpHeight) < CGFloat(screenHeight) ? (((self.screenHeight/2) - ((lblMessage.frame.origin.y + lblMessage.frame.size.height + viewFooterPopup.frame.size.height) / 2)) - 20) : 10 , width: viewPopUp.frame.size.width , height: popUpHeight)
        ...
        if CGFloat(popUpHeight) > CGFloat(screenHeight)
        {
            scrollPopup.contentSize = CGSize(width: screenWidth, height: CGFloat(popUpHeight) + 20)
        }
